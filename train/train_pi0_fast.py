...
class Pi0FastDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return (
            torch.tensor(self.states[idx], dtype=torch.float32),  # (4,)
            torch.tensor(self.tokens[idx], dtype=torch.long),     # (L,)
            torch.tensor(self.attn_mask[idx], dtype=torch.long),  # (L,)
            torch.tensor(self.tokens[idx], dtype=torch.long).clone()  # (L,)
        )
...
def train_pi0_fast():
    ...
    log_print(f"Hyperparameters: state_dim={state_dim}, vocab_size={vocab_size}, max_seq_len={max_seq_len}, embed_dim={embed_dim}, num_epochs={num_epochs}, batch_size={batch_size}, lr={lr}")
    ...
    dataset = Pi0FastDataset("data/training_pairs_fast.npz")
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (states_batch, token_seqs_batch, attn_mask_batch, targets_batch) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            logits = model(states_batch, input_seq, input_mask)  # (B, T-1, vocab_size)
